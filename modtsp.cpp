...
*/
  ...
  ModGA gapop = ModGA(pop_size, cutoff, mutate_prob, factory); 
  ...
  while (1) { //prev_eval == opt_tour) {
    ...
    if (winner->evaluate() != prev_eval) {
      ...
      cout << "Best of Gen " << (i+1) << " " << abs(winner->evaluate()) << endl;
  ...
  winner = (TSPChromosome*)gapop.get_best_chromosome();
  cout << "Generation: " << (i+1) << " Best Fit " << abs(winner->evaluate()) << endl;
